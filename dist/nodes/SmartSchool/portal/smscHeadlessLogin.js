...
async function handleClassicTotp(page, totpSecret) {
    ...
    if (!normalizedSecret) {
        throw new Error('TOTP is required but no TOTP secret was provided.');
    }
...
async function smscHeadlessLogin(creds) {
    ...
    if (!usernameInput) {
        throw new Error('Could not find a username input on the Smartschool login page.');
    }
    ...
    if (!passwordInput) {
        throw new Error('Could not find a password input on the Smartschool login page.');
    }
    ...
    try {
        await page.waitForSelector('#twoFaChooser, input[type="tel"], input[placeholder*="six" i], input[name*="otp" i]', {
            timeout: 30000,
        });
        await handleClassicTotp(page, creds.totpSecret);
    }
    catch (_) { }
